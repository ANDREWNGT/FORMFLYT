...
%ECI
lumelite1_true_ephem_eci = readtable(folder_path+"\Lumelite1_finite_Inertial_Position_Velocity.csv", "VariableNamingRule","preserve");
lumelite2_true_ephem_eci = readtable(folder_path+"\Lumelite2_finite_Inertial_Position_Velocity.csv", "VariableNamingRule","preserve");
lumelite3_true_ephem_eci = readtable(folder_path+"\Lumelite3_finite_Inertial_Position_Velocity.csv", "VariableNamingRule","preserve");
...
p2log = zeros(nSamples,3);
p3log = zeros(nSamples,3);
...
for N = 1 : nSamples
    ...
    % Cartesian coords.
    if N == 43200
        p1 = [4832793,-4835920,-1122900];
        v1 = [5293.561,5409.238,-471.66];
        %43200/30=1440, go to A1442
        p2 = [3253021,-6023860,-950701];
        v2 = [6595.626,3691.999,-803.6];
        p3 = [1195564,-6775550,-671578];
        v3 = [7387.186, 1412.664, -1092.82];
        ...
        vel2a(N,:) = v2;
        pos3a(N,:) = p3;
        vel3a(N,:) = v3; % 1 by 3 vector

    elseif N == 1252830
        p1 = [-3634360,5872824,444860.7];
        v1 = [-6408.97,-3880.25,-1224.99];
        %1252842/30=41761, go to A41763
        p2 = [-3620420,5870716,437691.8];
        v2 = [-6430.01,-3871.86,-1203.68];
        p3 = [-3536920,5922781,453646.6];
        v3 = [-6483.16,-3778.18,-1197.22];
...
fprintf("sat 3 intrack: %f km\n", posRIC3a(1,3)/1000)



fprintf("1st burn\n")
fprintf("sat 2 ECI (matlab): [%f, %f, %f] km\n", p2log(43199,:)/1000)
fprintf("sat 2 RIC (matlab): [%f, %f, %f] km\n", posRIC2a(43199,:)/1000)
fprintf("sat 3 ECI (matlab): [%f, %f, %f] km\n", p3log(43199,:)/1000)
fprintf("sat 3 RIC (matlab): [%f, %f, %f] km\n", posRIC3a(43199,:)/1000)

fprintf("2nd burn\n")
fprintf("sat 2 ECI (matlab): [%f, %f, %f] km\n", p2log(1252829,:)/1000)
fprintf("sat 2 RIC (matlab): [%f, %f, %f] km\n", posRIC2a(1252829,:)/1000)
fprintf("sat 3 ECI (matlab): [%f, %f, %f] km\n", p3log(1252829,:)/1000)
fprintf("sat 3 RIC (matlab): [%f, %f, %f] km\n", posRIC3a(1252829,:)/1000)
...
%Plot in eci frame
plot_absolute_error_eci(7, pos1a, lumelite1_true_ephem_eci, time_steps, tt, ephem_freq, every_nth_point, 1)
plot_absolute_error_eci(8, pos2a, lumelite2_true_ephem_eci, time_steps, tt, ephem_freq, every_nth_point, 2)
plot_absolute_error_eci(9, pos3a, lumelite3_true_ephem_eci, time_steps, tt, ephem_freq, every_nth_point, 3)
...
%RIC Lum2 and Lum3 wrt Lum1, relative displacements
...
% RIC absolute error comparison Lum2 and Lum3 wrt Lum1
...
function [] = plot_absolute_error_eci(figure_number, posa, lumelite_true_ephem, time_steps, tt, ephem_freq, every_nth_point, vehicle_number)
%%%Function to plot the absolute deviation between matlab and stk method
fh=figure(figure_number);
fh.WindowState = 'maximized';
...
diff_x = abs(posa(1:every_nth_point:end,1)-lumelite_true_ephem.('x (km)')(1:tt/ephem_freq+1).*1000);
diff_y = abs(posa(1:every_nth_point:end,2)-lumelite_true_ephem.('y (km)')(1:tt/ephem_freq+1).*1000);
diff_z = abs(posa(1:every_nth_point:end,3)-lumelite_true_ephem.('z (km)')(1:tt/ephem_freq+1).*1000);


x_plot = plot(time_steps(1,1:every_nth_point:end), diff_x, 'xr', 'LineWidth', 1);
y_plot = plot(time_steps(1,1:every_nth_point:end), diff_y, 'xb', 'LineWidth', 1);
z_plot = plot(time_steps(1,1:every_nth_point:end), diff_z, 'xg', 'LineWidth', 1);
