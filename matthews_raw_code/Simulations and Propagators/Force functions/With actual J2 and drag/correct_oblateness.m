...
function result = correct_oblateness(p)
    ...
    r2 = norm(p)^2;
    tmp = (p(3)^2)/r2;
    oblate_x = (3/2)*J2*fac*((R^2)/r5)*p(1)*(5*tmp-1);
    oblate_y = (3/2)*J2*fac*((R^2)/r5)*p(2)*(5*tmp-1);
    oblate_z = (3/2)*J2*fac*((R^2)/r5)*p(3)*(5*tmp-3);
