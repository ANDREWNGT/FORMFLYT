...
function f=RK4correctorbitJ2solver(a, e, o, i, w) %in terms of initial orbit elements, start plot from PERIGEE
    arguments
        a
        e
        o
    ...
    for i = 1:N % remember indexing 1 to N in matlab array!!
        ...
        kv2= correct_pert_A(Position(i,:)+(dt/2).*kp1);
        
        kp3= Velocity(i,:)+(dt/2).*kv2;
        kv3= correct_pert_A(Position(i,:)+(dt/2).*kp2);
        
        kp4= Velocity(i,:)+dt.*kv3;
        kv4= correct_pert_A(Position(i,:)+dt.*kp3);
