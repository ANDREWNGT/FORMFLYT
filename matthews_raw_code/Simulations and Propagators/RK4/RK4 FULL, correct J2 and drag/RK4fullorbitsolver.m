...
function f=RK4fullorbitsolver(a, e, o, i, w) %in terms of initial orbit elements, start plot from PERIGEE
    arguments
        a
        e
        o
    ...
    for i = 1:N % remember indexing 1 to N in matlab array!!
        ...
        % dvx/dt = g(x,y,z,vx,vy,vz,t)={g(x,y,z);Newton+oblate} +
        % {h(vx,vy,vz);drag}(and so on)
        
        kp1= Velocity(i,:);
        kv1= full_A(Position(i,:),Velocity(i,:));
        ...
        kv2= full_A(Position(i,:)+(dt/2).*kp1,Velocity(i,:)+(dt/2).*kv1);
        
        kp3= Velocity(i,:)+(dt/2).*kv2;
        kv3= full_A(Position(i,:)+(dt/2).*kp2,Velocity(i,:)+(dt/2).*kv2);
        
        kp4= Velocity(i,:)+dt.*kv3;
        kv4= full_A(Position(i,:)+dt.*kp3,Velocity(i,:)+dt.*kv3);
    ...
    plot3(Position(:,1),Position(:,2),Position(:,3), 'LineWidth',0.00001, 'Color', 'r')
    hold on
