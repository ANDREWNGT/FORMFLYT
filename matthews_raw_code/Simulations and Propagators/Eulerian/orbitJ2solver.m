...
function f=orbitJ2solver(a, e, o, i, w) %in terms of initial orbit elements, start plot from PERIGEE
    arguments
        a
        e
        o
    ...
    dt=2 %2 seconds
    A=-fac.*invsq([X0,Y0,Z0])+oblateness([X0,Y0,Z0]) %reminder elementwise, REMEMBER NEGATIVE SIGN!!
    ...
    for i = 1:N % remember indexing 1 to N in matlab array!!
        ...
        A=-fac.*invsq(Position(i+1,:)) + oblateness(Position(i+1,:))
        %physics law add in perturbing J2 acceleration
        %PHYSICS LAW HERE, of current position CENTRAL FORCE, 
    ...
    title('Orbit with J2 solved in ECI')
